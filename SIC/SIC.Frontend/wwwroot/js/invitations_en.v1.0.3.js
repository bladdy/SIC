...
// invitations.js
// ==============================
//Bug fix       → invitations.v1.0.1.js
//Nueva función → invitations.v1.1.0.js
//Cambio API    → invitations.v2.0.0.js
...
function mostrarDatosInvitacion(inv) {
    ...
    if (tipoEvent !== "Save the Date") {
        ...
        if (inv.numberAdults > 0) hayAdultos = true;
        if (inv.numberYouths > 0) hayJovenes = true;

        document.getElementById("invitados_mayores").innerText = `${inv.numberAdults} Adulto(s)`;

        const invAdultos = document.getElementById("invitados_mayores");
        if (hayAdultos) {
            invAdultos.innerText = `${inv.numberAdults} Adult(s)`;
        }
        else {
            invAdultos.style.display = "none";
        }

        const invJovenes = document.getElementById("invitados_jovenes");
        if (hayJovenes) {
            invJovenes.innerText = `${inv.numberYouths} Youth(s)`;
        }
        else {
        ...
        document.getElementById("invitados_menores").innerText = inv.numberChildren === 0
            ? "Adults only"
            : `${inv.numberChildren} Children`;

        llenarSelect("confirmadosadultos", inv.numberAdults, "Adult");
        llenarSelect("confirmadosjovenes", inv.numberYouths, "Young");
        llenarSelect("confirmadosmenores", inv.numberChildren, "Child");
    }
}
...
function fn_asistencia(valor) {
    ...
    if (valor === "s") {
        if (hayAdultos) selAdultos.style.display = "block";
...
function sendRespuesta() {
    if (!invitacionData) {
        alert("The invitation has not been loaded correctly.");
        return;
    }
    ...
    if (!asistira && !noAsistira && (tipoEvent !== "Save the Date")) {
        alert("Please select whether or not you will be attending the event.");
        return;
    }
    ...
    fetch(`${apiUrl}/api/invitations/confirm`, {
        ...
        .then((res) => {
            if (!res.ok) throw new Error("Error sending confirmation.");
            return res.json();
        })
        .then((data) => {
            if (asistira) {
                ...
                const mensaje = `I have already confirmed my attendance, thank you for the invitation.`;
    ...
    fetch(qrUrl)
        ...
        .then(data => {
            ...
            if (!data.qrBase64 || data.qrBase64 === "null") {
                console.error("The backend did not return a valid QR code");
                imgQr.src = "";
                return;
            }
            ...
            if (data.pdfBase64 && data.pdfBase64 !== "null") {
                ...
                enlaceDescarga.textContent = "Download PDF receipt";
            }
