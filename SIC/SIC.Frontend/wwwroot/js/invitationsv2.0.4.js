...
function mostrarGracias(data) {
    ...
    // Mostrar mesa asignada (invitacion → individual)
    var mesaEl = document.getElementById("mesa_asignada");
    if (!mesaEl) {
        mesaEl = document.createElement("div");
        mesaEl.id = "mesa_asignada";
        mesaEl.style.display = "none";
        var ref = document.getElementById("cont_descargaqr");
        if (ref && ref.parentNode) {
            ref.parentNode.insertBefore(mesaEl, ref);
        } else {
            var target = document.getElementById("mensaje_gracias_asistire") || document.getElementById("mensaje_gracias_noasistire");
            if (target && target.parentNode) {
                target.parentNode.insertBefore(mesaEl, target.nextSibling);
            }
        }
    }
    if (mesaEl) {
        console.log("[Mesa] invitacion.tablesEvents:", invitacion.tablesEvents);
        console.log("[Mesa] invitacion.guests:", invitacion.guests);
        var mesaNombre = null;
        if (invitacion.tablesEvents?.name) {
            mesaNombre = invitacion.tablesEvents.name;
        } else if (invitacion.guests) {
            var guestConMesa = invitacion.guests.find(g => g.tablesEventsId != null);
            console.log("[Mesa] guestConMesa:", guestConMesa);
            if (guestConMesa && guestConMesa.tablesEvents) {
                mesaNombre = guestConMesa.tablesEvents.name;
            } else if (guestConMesa) {
                mesaNombre = "Mesa #" + guestConMesa.tablesEventsId;
            }
        }
        console.log("[Mesa] resultado:", mesaNombre);
        if (mesaNombre) {
            mesaEl.textContent = "Mesa: " + mesaNombre;
            mesaEl.className = "fw-bold text-uppercase mt-3 text-black";
